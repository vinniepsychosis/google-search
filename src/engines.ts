  ...
  async function attempt(headless: boolean): Promise<SearchResponse> {
    ...
    try {
      ...
      const extract = async (remaining: number) => {
        const run = async () => {
          // extractEngineResults is passed as a FUNCTION; under tsx/esbuild its nested
          // named arrows get wrapped in __name(...), which throws "__name is not defined"
          // in the browser. Shim __name (an identity is fine — it only sets fn.name) via a
          // STRING evaluate that isn't itself esbuild-transformed.
          await page.evaluate(
            "globalThis.__name = globalThis.__name || function (f) { return f; };"
          );
          return page.evaluate(extractEngineResults, {
            engine,
