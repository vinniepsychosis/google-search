...
import { getGoogleSearchPageHtml, imageSearch, DEFAULT_STATE_FILE } from "./search.js";
...
program
  ...
  .action(async (query: string, options: CommandOptions & { getHtml?: boolean, saveHtml?: boolean, htmlOutput?: string, solve?: boolean, saveState?: boolean, images?: boolean }) => {
    try {
      ...
      // A one-time solve is most useful when its cleared session is shared with the
      // API/MCP servers (they read DEFAULT_STATE_FILE). If someone explicitly solves
      // against the legacy ./browser-state.json, redirect to the shared path.
      ...
      if (options.images) {
        // Image search (Google Images / udm=2) — Google-only, independent of --engine.
