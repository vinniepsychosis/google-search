...
- Result shape (google): `{ query, results[], answerBox?, sportsMatches?, weather?, peopleAlsoAsk?, relatedSearches?, pagination }` — see `src/types.ts`.
...
- **Two stealth layers, both on `playwright-extra` + `puppeteer-extra-plugin-stealth`** (kept separate on purpose): `src/browser.ts` exports the stealth-patched `chromium` used by the **Google** path (`search.ts` + servers), which additionally applies manual `navigator`/`window.chrome`/WebGL init-scripts **and a coherent geo fingerprint** (see Anti-bot below). `src/stealth.ts` (`launchStealthBrowser()` / `createStealthSession()`) serves the **non-Google** engines with the plugin only and **no** manual init-scripts — their *inconsistency* was what tripped Bing. Registering the plugin in either module is idempotent (same playwright-extra singleton).
